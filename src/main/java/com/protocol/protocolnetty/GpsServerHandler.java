...
import com.protocol.protocolnetty.proto.NetworkMessage;
import com.protocol.protocolnetty.proto.helper.BitUtil;
import com.protocol.protocolnetty.proto.helper.BufferUtil;
import com.protocol.protocolnetty.proto.helper.Checksum;
import com.protocol.protocolnetty.proto.helper.UnitsConverter;
import com.protocol.protocolnetty.proto.model.CellTower;
import com.protocol.protocolnetty.proto.model.Network;
import com.protocol.protocolnetty.proto.model.Position;
import com.protocol.protocolnetty.proto.session.DeviceSession;
...
public class GpsServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private void parseIdentification(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (channel != null) {
            ByteBuf responseBuf = channel.alloc().buffer();
    ...
    private String path;
    private final boolean connectionless=false;
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
        decode(ctx.channel(),ctx.channel().remoteAddress(), msg);
    }
    protected void channelr(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof ByteBuf) {
            ByteBuf byteBuf = (ByteBuf) msg;
            System.out.println(byteBuf.readableBytes());
            byte[] byteArray = new byte[byteBuf.readableBytes()];
            System.out.println("after array  : "+byteBuf.readableBytes());
            if (byteArray.length<1250){
                System.out.println("after check array lentgh  : "+byteBuf.readableBytes());
                ByteBuf responseBuf = ctx.alloc().buffer();
                responseBuf.writeBoolean(true);
                ctx.writeAndFlush(responseBuf);
            }
            else {
                String imei ="350612072451709" ;
                byteBuf.skipBytes(4);
                parseData(ctx.channel(), ctx.channel().remoteAddress(),byteBuf, 0,imei);
            }
        }
    }
    ...
    private void decodeLocation(Position position, ByteBuf buf, int codec, String model) {
        ...
        if (codec == CODEC_GH3000) {
            System.out.println("the if before adding lat and long");
...
//                    getConfig(), buf.readUnsignedShort(), buf.readUnsignedShort());
            ...
            System.out.println("positions affect");
        ...
        System.out.println(position.getString(Position.KEY_BATTERY));
    }
    ...
    private List<Position> parseData(
        ...
        for (int i = 0; i < count; i++) {
            Position position = new Position("Teltonika");
            if (codec == CODEC_13) {
                buf.readUnsignedByte(); // type
                int length = buf.readInt() - 4;
                System.out.println("length CODEC_13");
                ...
                System.out.println("decode serial");
                decodeSerial(channel, remoteAddress, null, position, buf);
            } else {
                decodeLocation(position, buf, codec, "FMB920");
            }

            if (!position.getOutdated() || !position.getAttributes().isEmpty()) {
                System.out.println("add positions to collection");
    ...
    protected Object decodeFrame(
            ChannelHandlerContext ctx, Channel channel, ByteBuf buf) throws Exception {

        while (buf.isReadable() && buf.getByte(buf.readerIndex()) == (byte) 0xff) {
            System.out.println("while : "+buf.readRetainedSlice(1));
    ...
    private Object decodeTcp(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        System.out.println("decode TCP: "+buf.readableBytes());
        if (buf.readableBytes() == 1 && buf.readUnsignedByte() == 0xff) {
            return null;
        } else if (buf.readableBytes() == 17) {
            parseIdentification(channel, remoteAddress, buf);
        } else {
            System.out.println("parseData call");
            buf.skipBytes(4);
        ...
        if (type == 0x0D) {
            ...
            if (subtype == 0x01) {
                ...
                if (photo.writableBytes() > 0) {
                    ...
                    try {
                        position.set(Position.KEY_IMAGE, writeFile(deviceSession.getUniqueId(), photo, "jpg"));
                    } finally {
                        photo.release();
                    }
                }
    ...
    public String writeFile(String uniqueId, ByteBuf buf, String extension) {
        if (path != null) {
            ...
            try (FileOutputStream output = new FileOutputStream(File.createTempFile(uniqueId, name));
